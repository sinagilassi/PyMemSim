...
def to_m(value: float, unit: str) -> float:
    """
    Convert a given value to meters (m) based on the specified unit.
    ...
    unit : str
        The unit of the input value. Supported units include 'm', 'cm', 'mm', 'ft', etc.

    Returns
    -------
    float
        The converted value in meters (m).
    ...
    try:
        ...
        return pycuc.convert_from_to(value, from_unit=unit, to_unit='m')
    except Exception as e:
        logging.error(f"Error converting {value} from {unit} to m: {e}")
